...
from apivoidv2_consts import (
    APIVOIDV2_CONNECTIVITY_ENDPOINT,
    APIVOIDV2_ERR_CONNECTIVITY_TEST,
    APIVOIDV2_SUCC_CONNECTIVITY_TEST,
)


class Apivoidv2Connector(BaseConnector):
    """
    Apivoidv2 connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = APIVOIDV2_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(APIVOIDV2_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(APIVOIDV2_SUCC_CONNECTIVITY_TEST)
